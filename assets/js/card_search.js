...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const filterItemsAll = (searchTerms) => {
    ...
    document.querySelectorAll("a.card-container").forEach(function (element) {
      ...
      while (iterator && (iterator.tagName !== "a" || iterator.classList == null || !iterator.classList.contains("card-container"))) {
  ...
  const filterItem = (searchTerm) => {
    ...
    if (CSS.highlights) {
      const { matchingElements, nonMatchingElements } = highlightSearchTerm({
        search: searchTerm,
        selector: ".card-container > div",
